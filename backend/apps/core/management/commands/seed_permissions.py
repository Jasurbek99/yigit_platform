...
# admin / director / export_manager / boss hold it via _ALL_PAGES, and
# document_team via its copy of export_manager's set. The two added here own NO
# TaskRule at all: a greenhouse_manager's whole queue is the `weekly_plan` kind
# and a seller's is `local_sell_plan`, both generated by code rather than by a
# rule, so the page's code-driven-kinds table is the only place either role's
# work is written down. Must stay in step with VISIBLE_ROLES in
