...
class CommandTests(TestCase):
    ...
    @patch('apps.transport.management.commands.poll_traccar_positions.sync_geofences', return_value=31)
    @patch('apps.transport.management.commands.poll_traccar_positions.sync_devices', return_value=95)
    def test_poll_reports_count(self, mock_sync_devices, mock_sync_geofences, mock_sync_positions):
        out = StringIO()
    ...
    @patch('apps.transport.management.commands.poll_traccar_positions.sync_geofences', return_value=31)
    @patch('apps.transport.management.commands.poll_traccar_positions.sync_devices', return_value=95)
    def test_poll_handles_traccar_unavailable_from_sync_positions(
