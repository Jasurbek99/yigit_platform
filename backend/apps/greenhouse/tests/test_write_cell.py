...
class TestWriteCell(TestCase):
    ...
    def test_second_write_reuses_existing_container_no_duplicate(self):
        first = self._client(self.manager).post(
            self.URL,
            {'block': self.block.id, 'entry_date': self.monday.isoformat(), 'plan_value': '100.00'},
            format='json',
        )
        ...
        second = self._client(self.admin).post(
            self.URL,
            {
                'block': self.block.id, 'entry_date': self.monday.isoformat(),
        ...
        self.assertEqual(
            WeeklyHarvestPlan.objects.filter(
                ...
                week_number=self.monday.isocalendar().week, year=self.monday.isocalendar().year,
            ).count(),
            1,
        )
        self.assertEqual(HarvestDayEntry.objects.filter(block=self.block, entry_date=self.monday).count(), 1)
    ...
    def test_refused_write_keeps_rows_that_already_existed(self):
        ...
        response = self._client(self.other_manager).post(
            self.URL,
            {'block': self.block.id, 'entry_date': self.sunday.isoformat(), 'plan_value': '99.00'},
            format='json',
        )
