...
|---|---|---|---|
| 1 | Boss: `/export/plan` — ввод и назначение плана, одно значение в ячейке | 3 | **не в git** |
| 2 | Водители: импорт 152 из Z_TIRWEB, вкладка «Водители» в `/admin/fleet`, выбор водителя в ячейке R27 | 3 | в git (21 авг) |
| 3 | `/export/pomidor-dukany` — план vs факт по блокам | 1 | **не в git** |
| 4 | Документы: красные заполненные значения + настройка полей страницы | 1 | **не в git** |
...
| 8b | Сезоны под капотом: резолвер сезона, 20 переведённых запросов, backfill FK, два слоя write-freeze — **браузером не проверяются**, только прогон `apps.core` / `tests_seasons` | ~30 | в git |
...
**Главное число: 45 из 83 записей — это одна фича «Сезоны».** Но кликами закрывается только 8a (~15 записей);
8b — это внутренняя механика, её закрывает прогон тестов, а не браузер.

Порядок: 1–4 (свежее, ещё никем не виденное) → 8a → 5–7 → 9–15. 8b — отдельно, прогоном тестов.
...
4. **Пробел на фронте:** в `WeeklyPlanGrid.tsx` (913 строк) четыре ролевых предиката — `isAdmin` (тягачи, намеренно без boss), `isAdminLike`, `canEditHarvest`, `planOnlyCells` — плюс правило `canEditActual = isAdminLike && !planOnlyCells`. Тестов на эту связку нет, а полноценный render-тест компонента на 913 строк с 10 хуками — дорого и хрупко. Предложение: вынести предикаты в `WeeklyPlanGrid.roles.ts` (как уже сделано с `HarvestCell.helpers.ts`, `PomidorDukany.helpers.ts`) и покрыть unit-тестами. Не делал — это правка продового кода, а не тестов.
