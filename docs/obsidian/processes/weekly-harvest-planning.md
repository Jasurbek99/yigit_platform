...
9. **Admin/boss direct edit in-week** (existing override-with-reason path): after writing, set `plan_baseline_value = value` and supersede any pending request on that cell. An admin's in-week **zero** (or clear) stores `plan_baseline_value = NULL`, not 0 — a 0 baseline would leave the cell unbounded for the rest of the week (rule 2).
...
- **Locking.** `request_plan_change` and `approve_plan_change` both take `select_for_update()` on the `HarvestDayEntry` row first, then touch the request row, and the request path reads the withdraw check, baseline and bound from that locked row, never the caller's in-memory copy. The admin direct-edit tail (write `plan_value` → audit → baseline reset → supersede) runs in one `transaction.atomic()`, so it also locks entry → request. One lock order everywhere means concurrent edits on a cell serialise instead of hitting `uq_pcr_one_pending` (500) or deadlocking.
- **Season close.** Requests still `pending` when a season closes stay `pending` in the archive. Approving them (or superseding them by a new edit) gets 409 from the season freeze. There is no auto-expiry, by design.
- **P3 alert.** The Monday 00:00 "CRITICAL: plan still missing" trigger counts a cell with a pending request as entered. A manager who fills empty cells after the week starts has `plan_value` still NULL while approval waits, and would otherwise be flagged.
- **Bad input** (non-numeric, negative, ≥ 100,000,000, `reason`/`note` over 500 characters) is a 400, never a 500.
...
Inside the `planOnly` cell branch, once the cell's plan week has started, a non-admin editor sees the allowed ± range under the input (`planChangeRange()` in `HarvestCell.helpers.ts`, mirroring the backend bound; the server stays authoritative) and an out-of-range blur is refused client-side with a toast, no request sent. A cell with a pending request shows a small yellow badge under the value — `→ 11,500 (+15%) ⏳` (`PendingChangeBadge` in `HarvestCell.tsx`) — with the requester's name in a tooltip. Saving a plan value that routes to approval (202 response) shows an info toast ("Sent for approval") instead of the normal save toast. Typing the approved value back over a pending change still sends the save (normally an unchanged value is skipped) and withdraws the request. Only typing counts: entering the cell and leaving it untouched (a click away, or Tab/arrow traversal) sends nothing; the 200 looks like a plain save, so the grid checks the cell's state from before the save and shows "Pending change withdrawn". A refused in-week clear shows its own toast (`plan.change_clear_refused`).

The revision log/approval queue is a **toolbar button with a pending-count badge** (`plan.changes_button`, `Badge` + `DiffOutlined`; the count is season-wide, not the selected week, because from Friday the grid defaults to next week) that opens `components/PlanChangeRequestsDrawer.tsx` — not a tab, since `WeeklyPlanGrid` has none. Columns: block, day, baseline, current, requested, ±% (green up / red down), requested by, reason, status, decided by/at, note. Filters: pending/all status, current-week/all-weeks scope (defaults: pending, all weeks). A failed list load shows an error alert instead of the empty state. Approve / Reject (with an optional note) show only for `export_manager`/`admin`/`boss` (`canDecidePlanChanges` in `WeeklyPlanGrid.roles.ts`); everyone else sees it read-only. `?changes=1` in the URL — the notification link's format — opens the drawer on mount, and again if the param arrives after mount (in-app navigation).
