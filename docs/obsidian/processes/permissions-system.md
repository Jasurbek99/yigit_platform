...
> hardcoded set) would have 403'd every write. Both were widened on 2026-08-20, and on
> **2026-09-03 both fleet screens were registered as page codes** (`transport.fleet`,
> `transport.map`) after the owner asked why neither appeared on the permission screen — see
> [[../screens/fleet-admin#Access]] and [[fleet-map#Fleet Map Page]]. `/worklog`, `/team/kpi`
> and `/admin/process-links` still carry inline `roles` arrays. `AppLayout.menuGroups.test.tsx`
> could not catch the boss bug, because its fixture sets `is_superuser: true` and takes the
> bypass branch; the guard is a test that renders a **non-superuser** boss. When auditing boss
> access, grep for inline `roles={[` / `roles: [` alongside the page_code registry.
>
...
- `worklog` (Work Hours), `team_kpi` (Team Leaderboard) — registered 2026-09-16 (core migration `0051_worklog_team_kpi_page_perms`); both were hardcoded 15-role `roles` arrays before that, so they appeared on neither `/admin/permissions` nor `/admin/staff-access`. Flat codes, so they group under **Main** in the admin UI. Seeded visible to **every role** (in `_UNIVERSAL`), keeping radical transparency as the default while making it revocable. **Page row only** — the worklog and team-KPI endpoints stay `IsAuthenticated` (the header `WorklogChip` reads `/core/worklog/me/` for everyone), so unticking hides the nav entry and the route, not the API.
...
- `transport.map` (Fleet Map), `transport.fleet` (Fleet Management) — registered 2026-09-03 (core migration `0039_fleet_page_perms`); both were hardcoded `roles` arrays before that and so never appeared on `/admin/permissions`. **Non-`admin.` prefix on purpose**: `warehouse_chief` and the loading head/deputy hold `transport.fleet`, and every `admin.*` code is stripped from `director`/`export_manager` by `_ALL_PAGES - _ALL_ADMIN` (AD-15). Unlike most page codes these gate the backend too — `CanViewFleetMap` reads `transport.map` (the Fleet Map's live-positions endpoint 403s without it) and `CanEditFleet` reads `transport.fleet` (truck/trailer/driver **writes**; reads stay open, the Sheet's pickers need them). Seeded defaults reproduce the old hardcoded sets exactly: `transport.map` for every role but `seller`, `transport.fleet` for management plus `warehouse_chief`/`loading_dept_head`/`loading_dept_head_deputy`.
...
>
> **Registering a code for a page that already exists is step 4 with teeth.** Production runs `migrate`, not `seed_permissions`, so an *existing* screen that gains a page_code needs a data migration in the same release or it goes dark for every non-superuser the moment the bundle deploys. `seed_permissions` cannot heal it later either — it only `get_or_create`s, so it never flips a row someone already has (finding F13). `core/0039_fleet_page_perms` is the worked example: it writes both fleet codes for all 15 roles, `get_or_create` so a re-run cannot stomp an admin's toggle, guarded on a `test_`-prefixed DB name rather than `DJANGO_TESTING` (see `core/0033`'s docstring for why that distinction matters).
...
- `fleet` (added 2026-09-03, core migration `0040_fleet_resource`) — the truck-head / trailer / driver catalog behind the Fleet Management screen. All-or-nothing (no `RESOURCE_FIELDS` entry). Gates **writes** on `TruckHeadViewSet` / `TrailerViewSet` / `DriverViewSet` through `resource_write_permission('fleet')`, so **reads stay open to every authenticated user** — the truck / trailer / driver pickers on the Sheet and the shipment drawer list the same catalog, exactly like `packing_template`. Defaults are **view+create+edit, no delete** for `admin` / `director` / `export_manager` / `boss` / `warehouse_chief` / `loading_dept_head` / `loading_dept_head_deputy` (the old hardcoded `SHIPMENT_EDITOR_ROLES` set); `can_delete` is `False` for everyone because no `destroy` action exists to gate — rows are deactivated, not removed. `boss` needs an explicit row for the same reason `sheet_row_setting` did: the `_ALL_RESOURCES` wildcard only fires under `seed_permissions`, which production never runs, and `core/0033` enumerated the registry as it stood before `fleet` existed. **This is the resource half of a deliberate page/resource split**: the `transport.fleet` *page* row decides who SEES the Fleet Management screen, the `fleet` *resource* decides who may WRITE — the same division every other admin screen uses, so the Resources tab is where fleet editing is granted.
