...
- **Auto-complete (lazy)**: resolved on the `/me/tasks/` read path (`resolve_all_open_weekly_plan_tasks`), NOT on plan-save — greenhouse (where `set_plan_value` lives) may not call into export. To keep latency low the plan grid's day-entry mutation invalidates the `my-tasks` query, forcing a `/me/tasks/` refetch right after a cell save (otherwise worst-case latency = the 60 s nav-badge poll). A block's task flips to DONE once its **Mon–Sat** plan cells are filled (≥1 `HarvestDayEntry` row in Mon–Sat and none with `plan_value IS NULL`; explicit `0` counts as filled). **Sunday is excluded** — it is not measured, so a blank Sunday cell never blocks completion. `python manage.py resolve_weekly_plan_tasks` runs the same pass from cron/backfill.
  **The read-path pass is global, not per-caller** (fixed 2026-09-01). It used to resolve only `request.user`'s own tasks, but the grid is routinely filled by an admin or export_manager on a manager's behalf — and that manager may never open this screen — so the reminder never flipped to DONE. 35 such rows had accumulated across W25/W27/W28 of 2026, and all 12 open W36 tasks had complete grids. Any user's `/me/tasks/` load now clears every resolvable task; `completed_by` is still credited to the **assignee**, not to whoever triggered the read. Cost is bounded: generation covers only the current and next ISO week (~15 tasks each), each check is two indexed `exists()` calls on `(block_id, entry_date)`, and the open set drains as it resolves.
