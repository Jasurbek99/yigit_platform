...
**Comment (legacy composer)** (`POST /shipments/{id}/comment/`) body `{"content": "..."}` — gated on **`shipment_comment.can_create`**, not `shipment.can_create`. Backward-compatible endpoint that delegates to the same `create_comment()` service as `CommentViewSet`, so it must answer to the same resource; until 2026-09-01 it inherited the ViewSet's `shipment` resource_code and 403'd `document_team`, `transport`, `sales_rep`, `finansist` and `weight_master` — the five roles granted commenting (FINDINGS_BACKLOG F18). Used by `CommentComposer.tsx` on `ShipmentActivityLog`; the Sheet's richer comment UI posts to `/export/comments/` instead. Returns 201 + the shipment detail.

**Swap fields** (`POST /shipments/{a_id}/swap/`) body `{"other_id": <int>, "fields": [...]}` — gated on **`shipment.can_edit`** (`resource_edit_permission`), not `can_create`: a swap edits two shipments. **Which fields** may be exchanged is decided in the method body, per field, by `can_edit_sheet_field`, and a refusal there returns 403 **naming the field**. Until 2026-09-01 the coarse `can_create` gate ran first and refused `document_team`, `transport`, `sales_rep`, `finansist` and `weight_master` with DRF's generic message, so that loop never ran for them — and the Sheet toolbar's Swap button had no role gate, so they all saw it (FINDINGS_BACKLOG F19). The button now carries `canDo(user, 'shipment', 'edit')`. Both rows are locked in pk order with `select_for_update`, and shipment B gets its own closed-season guard because only A came through `get_object()`.

**Transition** (`POST /shipments/{id}/transition/`) body `{"new_status": "<code>", "comment": "optional"}` — gated on **`shipment.can_edit`**, not `can_create`. `get_permissions()` swaps `DynamicResourcePermission` for `resource_edit_permission('shipment')` on this action: a transition **edits** a shipment, and POST→`can_create` is 0 for `document_team`, `transport`, `sales_rep` and `finansist`, the roles that own 10 of the 11 lifecycle edges, so before 2026-09-01 only `export_manager`/`director`/`boss` could drive a step by hand (ROLE_ACCESS_AUDIT F12). **Which edge** a caller may walk is decided further in, by `transition_to()`'s per-edge role list — reaching the endpoint is not permission to move the shipment. `loading_dept_head` holds `can_edit`, owns no edge, and is refused there (FINDINGS_BACKLOG P5). `'cancelled'` is rejected with 400 — use `/cancel/`. `ShipmentBulkTransitionModal` calls this endpoint once per selected row.
...
| GET/POST | `/api/v1/export/customs-expense-categories/` | CustomsExpenseCategoryViewSet | `useCustomsExpenseCategories`, `useCreateCustomsExpenseCategory` | AdvancesTracker (expense modal) |
...
| GET/POST/PATCH | `/api/v1/transport/truck-heads/` `/truck-heads/{id}/` | TruckHeadViewSet — returns **`missing_details`** (`truck_model`, `tech_passport_scan`) | `useTruckHeads` (`useFleet`, read-only); `useAdminTruckHeads`/`useAdminCreateTruckHead`/`useUpdateTruckHead` (`useFleetAdmin`) | `ShipmentTruckSelector` (ShipmentDetail + edit drawer), FleetAdminPage (`/admin/fleet`) |
...
| GET/POST/PATCH | `/api/v1/transport/drivers/` `/drivers/{id}/` | DriverViewSet — returns **`missing_details`** (`passport_serial`, `passport_issue_date`, `passport_scan`) to **every** role, while the passport values themselves stay on `DriverAdminSerializer` | `useDrivers` (`useFleet`, active-only picker feed, read-only); `useAdminDrivers`/`useAdminCreateDriver`/`useUpdateDriver` (`useFleetAdmin`, incl. inactive) | `SheetDriverSelectEditor` (Sheet R27), FleetAdminPage Drivers tab (`/admin/fleet`) |
