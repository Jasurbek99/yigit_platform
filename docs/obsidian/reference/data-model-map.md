...
| **ExportFirm** | code, name_tk/ru/en, legal_type (FK, nullable), name_bare_tk/ru/en, patent_series/patent_number/patent_date (sole proprietors only), tax_code, is_active, is_gapy_satys | YGT's export legal entities (25). `name_*` keeps the stored string with the legal form baked in; `name_bare_*` is the same name with the form and quotes stripped. |
...
| **Driver** | name (100, Cyrillic collation), phone (30, nullable), logo_ref (100), driver_logo_code (120), is_active | Driver registry — 152 rows, ids 5–158, `id` preserved from `Z_TIRWEB.drivers`. `phone` is NULL for every row (source has none, and it is excluded from the upsert `defaults` so an operator entry would survive a re-import). `driver_logo_code` is the Logo accounting code and the reliable same-person key (names are not — see the fleet-map table); duplicates sharing a code are deactivated by the import, never deleted. `Shipment.driver_id` points here but is **NULL on all shipments** — driver identity is still the free text `driver_name`/`driver_phone`. See the open dropdown-binding defect in [[../processes/fleet-map#Drivers vs Shipment.driver_id]]. |
