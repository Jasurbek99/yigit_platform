...
- **Nothing is persisted.** This is a display-time reorder; `UserSheetRowPref` is never written. The per-row up/down reorder control is therefore **disabled** in this variant: `handleReorder` builds its payload from the array currently on screen, so one nudge would save the whole topic order as that user's personal row order. Hiding a row still works — it names one row id and doesn't depend on the order. (It briefly did not: the first cut derived `rowHasId` from the reorder gate, and hide is gated on `rowHasId` too. The gate now lives in a separate `canReorderRow`.)
