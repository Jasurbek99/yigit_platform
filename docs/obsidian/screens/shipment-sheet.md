...
- **The style controls** (the shared `SheetRowStyleControls` component, identical to the admin tab): background + font color (on one line), font weight, font style, font family, font size. (Width + alignment were removed as unusable.) Shown only to **admin / director / export_manager / superuser** (gated by `canEditRowStyle` in `SheetGrid`; the backend PATCH on `/admin/sheet-rows/{id}/` enforces the same set via the `sheet_row_setting` resource, see the Sheet Rows Admin endpoint section below).
...
`/api/v1/export/admin/sheet-rows/` — managed by `SheetRowSettingViewSet`. Auth: `resource_code = 'sheet_row_setting'` (2026-09-02, core migration 0038) — `admin` / `director` / `export_manager` only, via `DynamicResourcePermission`. Previously declared `resource_code = 'shipment'`, which let `document_team`, `transport`, `sales_rep`, `finansist` and `weight_master` reach it too (only the hidden admin page kept them off it) — see [[../roles/roles-matrix#Resource CRUD Matrix|roles-matrix]] footnote 8 and [[../processes/permissions-system]].
