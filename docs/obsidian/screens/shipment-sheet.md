...
## Hiding the "Who" column

Settings modal → **Show the "Who" column** (on by default). Column B names the person who owns each row; hiding it hands its 120 px back to the shipment columns — worth roughly one extra truck on a 1600 px screen. State: `whoColumnHidden` in `sheetStore`, persisted per browser to `localStorage` under `ygt-sheet-who-hidden`, like zoom/freeze/variant. Applies to both design variants.

**Only the column is hidden, never the data.** `default_who_key` still drives role-band grouping (`sheetRoleBlocks.ts`) and field permissions; in Classic the role bands keep naming each owner above their run, so ownership stays visible even with the column off. In the iOS variant the bands are topic sections, so hiding the column does remove the last on-screen trace of who owns a row — a fair trade for the width, but worth knowing.

Implementation note: the column becomes a **zero-width slot, not a removed one**. `scaleSheetLayout(zoom, variant, whoColumnHidden)` returns `colWho: 0`, and because `frozenLeftTotal` and the Field column's sticky `left` are both sums over those widths, everything shifts correctly with no other arithmetic. Keeping the 3-slot model matters: `frozenColCount` is expressed in it (1 = Row #, 2 = Who, 3 = Field name), so renumbering would silently change what every stored freeze value means. The cell itself is not rendered at all when hidden — a zero-width `div` would still paint its 1 px right border as a stray line — and the freeze-columns picker drops the now-meaningless "After column 2 — Who" option while leaving the other option values untouched.

## Design variant (skin)

The Sheet ships **two visual variants of the same grid**. Settings modal → **Design**: `Classic` (default) or `iOS (Tırlar)`. The iOS variant reproduces the Sera Bütçe app's *Tır Takip → Tırlar* screen — both its look and its topic row order (below) — that screen is the same transposed shape as this one (fields as rows, trucks as columns), so only the chrome differs: rounded pill cells on a light-gray canvas, a light column-header band instead of the dark one, softened role bands, and an emerald focus ring.

**It is a skin, not a second grid.** Both variants render the identical `SheetGrid` — the same rows, columns, payload, edit guards and permission checks. There is no forked component and no duplicated `isCellEditable` call. `SheetGrid` puts `sheet-grid--<variant>` on the scroll container and every rule in `components/sheet/SheetVariantIos.css` is scoped under `.sheet-grid--ios`; deleting that file restores the classic sheet exactly.
...
- **The skin never sets a cell `background`, and never uses `!important` on one.** Cell backgrounds carry data: per-cell colors (inline style), column tints (`.sheet-col-tinted`), the gapy-satyş tint and row-accent styles. The pill shape is produced with a 3 px transparent border plus `background-clip: padding-box`, so whatever background the cell already has is painted rounded and inset instead of edge-to-edge. An `!important` background here would silently erase the per-cell color feature.

### Topic row order (iOS variant only)

The iOS variant also **re-groups the rows the way the reference screen does** — by topic instead of by owner. Six sections, in the reference's order: 📋 General, 📦 Cargo & customs, 🚛 Transport, ⏱ Loading times, 🛣 Road & border, 📊 Sales & report. The mapping table is `TOPIC_SECTIONS` in `components/sheet/sheetTopicOrder.ts`; all 45 live rows are filed. **Inside** a section the reference's own sequence is followed exactly — the first cut grouped the fields correctly but ordered them within each section by judgement, which is not the same screen. Each section's comment quotes the reference sequence it mirrors, and a test (`follows the reference screen's sequence inside each section`) filters out the YGT-only fields and asserts the rest keep that relative order. A YGT-only field sits next to its nearest sibling in the reference (`customs_entry_at` after `dest_entry_at`, `weight_to_load_kg` after `weight_net`) or, with no sibling, at the end of its section.

The two groupings are mutually exclusive: a topic order interleaves owners, which trips the owner-contiguity gate in `markRoleBands` and would suppress every role band. So `applyTopicOrder()` emits its **own** bands in the same `IRoleBand` shape — `SheetRoleBandRow` renders them unchanged, and the grid uses them instead of the role bands while the variant is on. Classic keeps the role bands and the payload order, untouched.

Three properties this order must keep, all covered by tests:

- **No row is ever dropped.** A `field_key` the table doesn't name — every admin-added custom row, and any field added to `sheet_rows.py` before the table is updated — falls through to a trailing 📎 Other section in its original relative order. `backend/apps/export/tests_sheet_topic_order.py` reads *both* sources (the Python row list and the TypeScript table) and fails if a live row has no section, if a section names a dead field, or if a field is filed twice — the only cross-language drift alarm available, since neither file can see the other.
- **Rearranging it never touches the classic order.** The topic order can be rearranged with the same up/down arrows and drag handles as Classic, but through a completely different path: `handleTopicMove` → `moveRowInTopicOrder()` → `sheetStore.iosRowOrder`, a browser-local map of *section label key → field_keys in order* (`localStorage: ygt-sheet-ios-order`). It never calls `onReorder` and never PATCHes `/export/user/sheet-preferences/`, so the classic personal row order and its role bands are untouched — verified by watching the network during a move. It also doesn't need `fieldKeyToRowId`: a row with no `SheetRowSetting` id still moves, because nothing is being named in a server payload. **Section membership follows the drop** — a row joins the section of whatever row it lands on, which is what makes one gesture serve both within-section and cross-section moves; emptying a section removes its band, and the freeze re-snaps. Settings → *Row order (iOS design)* → **Reset to the default topic order** appears whenever an override exists. A field the override never mentions still renders, in its default section, so a row added to `sheet_rows.py` later is never swallowed by a stale override.
- **Hiding a row still works here** — it names one row id and doesn't depend on the order. (It briefly did not: the first cut derived `rowHasId` from the reorder gate, and hide is gated on `rowHasId` too. The gate now lives in a separate `canReorderRow`.)
- **The freeze picker matches the order on screen.** In this variant the Settings modal's *Freeze rows up to* list offers one option per section (`Through 📦 Cargo & customs`) instead of one per row — a row-named option would set a count that means a different row in the displayed order. The option values are exactly the section boundaries the grid snaps to, so what the picker says and what the grid does agree. `frozenRowCount` itself is still one shared setting across both variants (like zoom): the number means "the first N rows on screen" in both, but those are different rows, so changing it in one variant moves the freeze line in the other.
- **The freeze never splits a section.** Bands render only in the scrollable section, so a freeze line landing mid-section would swallow that section's band and leave its rows unheaded. `sectionAlignedFreeze()` snaps the count down to the section it lands in (the default 5 becomes 4 — the whole General section), so the frozen block is always a whole number of sections.

`SheetCellEditor` takes the variant as a **prop** (default `classic`) rather than reading the store, because it is also rendered outside the Sheet by `SelfBoardShipmentFieldList` — where none of the skin's `.sheet-grid--ios` rules apply. Reading the store there would let a preference set on the Sheet resize the Self Board drawer's inputs, unskinned. `SheetGrid` passes `variant={sheetVariant}`; every other call site keeps classic sizing. Locked by two tests in `SheetCellEditor.test.tsx`.

A per-row custom cell width (`SheetRowSetting.style.width`, set from the Sheet Rows Style popover) is multiplied by the variant's column density as well as by zoom — `scaleSheetLayout` returns `density` for exactly this. Scaling by zoom alone would leave that one row's cells narrower than its column slot in the iOS variant.

Not carried over from Sera in v1: the per-column source badges (🤖 Awtomatik / 📦 Gaplama / 📋 Export Bölüm) and the status pill in the column header. Those are additional *data* in the header, not styling, so they are out of scope for a skin.
...
Grouping the remaining 40 rows by role yields **6 blocks**, ordered by first appearance in the server order: `transport` (10: vehicle_condition/Logist + Haltac 2 + Mergen 1 + the generic `transport` who-key 6), `document_team` (6: transport_docs_given_at/documents_status/Sirin + firm_splits/Sulgun + document_note/customs_exit_at/Sirin + customs_clearance_planned_day/Sirin), `export_manager` (5: export_manager_note/firm_contracts/packing/is_gapy_satys/Gadam + sales_report_date/Aganazar), `loading_dept_head` (10: export_code/block_sources/harvest_status/warehouse_note/loading_started_at/loading_ended_at/weight_to_load_kg/weight_net/variety/harvest_date, all Soltanmyrat), `finansist` (1, Babageldi), `sales_rep` (8, Arap). `transport` leads because `vehicle_condition` — released from the old positional pin (2026-08-27) — is the first row after the 5 truly-pinned ones. Blocks are stable — rows keep their relative order inside a block; when two who-keys share a role (e.g. Haltac and Mergen, both `transport`), the block is emitted at the position of whichever appears **first** in the server order, and later same-role rows merge into it from wherever they were.

**Grouping can be switched off (2026-09-12).** Settings modal → **Group rows by role**, on by default. Off, the rows render in the admin order (`global_position`, the `#` column, finally reads monotonically) and **no bands render at all** — the switch drives both halves, because the order and the bands are computed in two different places: `groupRowsByOwner()` is gated in `ShipmentSheet.tsx` and the `markRoleBands()` call is gated in `SheetGrid.tsx`. `markRoleBands` itself is untouched — its contiguity gate is load-bearing — the call site returns an all-null array instead. State is `sheetStore.groupRowsByRole`, persisted per browser under `ygt-sheet-group-by-role`; an absent key means on, so the grouped view stays everyone's default.

Two deliberate limits. **A user who has a personal row order sees only the bands disappear** — their order already won the `hasPersonalRowOrder` condition, so there is nothing for the switch to ungroup; "Reset to role blocks" is still the way back to the grouped order. **The switch is hidden in the iOS variant**, where the bands *are* the topic sections and the order is the topic order: `topicOrder` takes precedence in both gated expressions, so turning grouping off in Classic and switching to iOS leaves that variant exactly as it was.
...
- **The style controls** (the shared `SheetRowStyleControls` component, identical to the admin tab): background + font color (on one line), font weight, font style, font family, font size. (Width + alignment were removed as unusable.) Shown only to **admin / director / export_manager / superuser** — `canEditRowStyle` in `SheetGrid` is a hardcoded frontend role literal, not derived from `canDo`/`sheet_row_setting` at all. **No longer the same set as the backend gate** (checked 2026-09-03): the backend PATCH on `/admin/sheet-rows/{id}/` gates on the `sheet_row_setting` resource, which now also includes `boss` (see the Sheet Rows Admin endpoint section below) — `boss` therefore has the backend permission to save a style change but the frontend never shows him the controls to make one. Untouched by this branch; not fixed here.
...
`/api/v1/export/admin/sheet-rows/` — managed by `SheetRowSettingViewSet`. Auth: `resource_code = 'sheet_row_setting'` (2026-09-02, core migration 0038) — `admin` / `director` / `export_manager` / `boss`, via `DynamicResourcePermission` (`boss` was briefly missing from the migration's `ROLES` list, fixed 2026-09-03 — see the Row access tab section above and `docs/FINDINGS_BACKLOG.md` F22). Previously declared `resource_code = 'shipment'`, which let `document_team`, `transport`, `sales_rep`, `finansist` and `weight_master` reach it too (only the hidden admin page kept them off it) — see [[../roles/roles-matrix#Resource CRUD Matrix|roles-matrix]] footnote 8 and [[../processes/permissions-system]].
...
## Live sync — other users' edits (2026-09-02)

Before this, the Sheet never showed anyone else's changes: `useShipmentSheet` has `staleTime: 30_000` and no `refetchInterval`, so a second person's edit only appeared on a page reload. Cross-user sync was never built — it was not a regression.

The server now broadcasts a **poke** over the existing presence WebSocket whenever a shipment, comment or task is written: ids and the author, never row data (see [[../processes/realtime-presence#Sheet change pokes]]). [`useSheetLiveSync`](../../../frontend/src/hooks/useSheetLiveSync.ts), mounted on this page only, turns that into an invalidate of `['shipments','sheet',seasonId]`. Typical latency ~0.5–1 s.

**Three things it deliberately refuses to do**, each protecting something that already works:

| Guard | Why |
|---|---|
| **Skips your own pokes** (`by_user_id` vs the cached `/auth/me/` id) | `useShipmentPatch` already folds the PATCH response into the cache and `invalidateExceptSheet` exists precisely to stop every keystroke refetching the unpaginated season. Since your own writes are the frequent case and other people's are rare, this is what keeps the whole feature cheap. |
| **Waits while one of your writes is in flight** (`queryClient.isMutating()`) | `useShipmentPatch.onMutate` calls `cancelQueries(['shipments'])`, which would abort the refetch mid-air; and a GET issued before your PATCH commits comes back **without** your value and stomps the optimistic cache — the cell visibly reverts. |
| **Waits while a cell editor is open** (`sheetStore.editingCell`) — **with no time limit** | Typed text is safe (the editors are uncontrolled inputs with stable React keys), but the Sheet is transposed: a shipment someone else creates lands at the **left edge** and shifts every column right, under your cursor. Product decision (2026-09-02): calm beats freshness here. The poke is **held, never dropped**, and lands the moment you close the editor. |

Both guards are read imperatively, never through a reactive subscription — `ShipmentSheet` re-renders the non-memoized `SheetGrid`, so a subscription would cost a full grid render per keystroke.

Bursts are coalesced over 400 ms. After invalidating, the hook confirms via `getQueryState(...).isInvalidated === false` and retries (up to ~10 s) if a concurrent `cancelQueries` swallowed the refetch — `invalidateQueries` resolves on cancellation, so its promise proves nothing. The pending batch is a single mutable object shared across the `await`, so the flush re-checks identity before touching it and re-sweeps ids that merged in mid-flight; a poke that arrives during an in-flight refetch was **not** covered by that refetch, because its write landed after the GET went out. A reconnect triggers one blind full refetch, because frames sent while the socket was down are gone for good.

Creates, deletes and reorders need no special handling: the refetch returns the authoritative row set, so a new column simply appears at the left edge and a deleted one disappears.
...
- **Live sync does not refresh your own second tab.** The self-skip compares the poke's `by_user_id` to the signed-in user, not to a socket — so two tabs of the same account will not update each other. The server cannot do better: the poke originates from an HTTP request that has no idea which socket you are on. `useUserSheetPrefsBroadcast` (BroadcastChannel) is the existing cross-tab precedent if this ever matters. Related: writes that do **not** come through the API (management commands, cron, direct DB) never poke at all, and a poke for a shipment in another season costs viewers of that other season one wasted refetch.
