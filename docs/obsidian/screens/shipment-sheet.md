...
- **Ctrl/⌘+X — cut** — copy, then `clearCell` the source immediately (cut = copy + clear), gated by the same editability + `isClearableField` checks as Delete. Non-clearable cells (`cargo_code`, computed flags, bool dropdowns) copy only.
