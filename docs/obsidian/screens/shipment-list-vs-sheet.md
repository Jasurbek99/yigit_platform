...
| Weights | `box_count`, `pallet_count`, `packaging_kg`, `weight_to_load_kg` | — | ✓ | ✓ |

> **Note:** `weight_to_load_kg` holds the (unofficial) tonnage that **must be loaded** and is displayed as **"Weight to load" / "Ýüküň agramy"** everywhere (Sheet row 36, List column, Detail field row). It was called `rejected_weight_kg` until 2026-09-12 (export migrations 0068/0069 renamed the column, the API field and the Sheet `field_key`); there is no "rejected weight" concept on the shipment. (The separate sales-report `weight_rejected_kg`, db_column `waste_kg`, is unrelated — it is waste recorded during the sale.)
