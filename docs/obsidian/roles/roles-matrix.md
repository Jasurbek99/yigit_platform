...
> **Fleet catalog — `VCU`, never `D`, and reads are open to everyone.** The `fleet` resource (added 2026-09-03, `core/0040_fleet_resource`) gates **writes** to truck heads, trailers and drivers; no cell carries `D` because none of the three ViewSets expose `destroy` at all — `Shipment.driver_id` is a loose integer with no FK, so rows are deactivated (`is_active=False`, an edit), never deleted. `-` here means "no writes", not "no reads": the list endpoints stay open to every authenticated user because the truck / trailer / driver pickers on the Sheet and the shipment drawer read the same catalog (`resource_write_permission`, the same read/write split `packing_template` uses). `loading_dept_head` and `loading_dept_head_deputy` are not columns in this table; both mirror `warehouse_chief` and hold `VCU`. The grants reproduce the hardcoded `SHIPMENT_EDITOR_ROLES` set the fleet ViewSets used before the resource existed. Pairs with the `transport.fleet` **page** row above: the page decides who sees the screen, this decides who may write.
>
> **Fleet Map / Fleet Management — page codes only since 2026-09-03 (`core/0039_fleet_page_perms`).** Both screens existed long before these rows: they were gated by hardcoded `roles` arrays in `App.tsx`/`AppLayout.tsx` plus `FLEET_MAP_DENIED_ROLES` / `SHIPMENT_EDITOR_ROLES` in `apps/transport/permissions.py`, which the matrix never consulted — so neither page appeared on `/admin/permissions` and changing who held them meant a deploy. The cells above are the seeded defaults, which reproduce those hardcoded sets exactly (nobody's access changed): the map for every role but `seller` (owner request 2026-08-23 — his `is_visible=False` row is written explicitly so the checkbox exists and the decision is reversible without a migration), fleet CRUD for management plus `warehouse_chief` and the loading head/deputy (`loading_dept_head_deputy` is not a column here; it mirrors `loading_dept_head`). Unlike most page rows these also gate the backend: `CanViewFleetMap` reads `transport.map`, so revoking it 403s `GET /transport/live-positions/`, and `CanEditFleet` reads `transport.fleet`, so revoking it 403s truck/trailer/driver **writes** — reads stay open to everyone, because the Sheet and shipment-drawer pickers list the same catalog. See [[../screens/fleet-admin#Access]] and [[../processes/fleet-map#Fleet Map Page]].
>
