...
| Admin Pages (Users, Firms, Seasons, Blocks, Customers, Truck Dest, Shipment Settings) | Y | - | - | Y | - | - | - | - | - | - | - |
...
>
> **`boss` widened 2026-08-05** from read-only/dashboard-only to full page visibility — every row above **except the Permission Matrix page**, which is excluded because `_AdminOnlyPermission` 403s even GET for non-admins, so the entry would be a dead menu item (AD-15 intact). The remaining Admin Pages run against AD-15's grain (`director`/`export_manager` are explicitly denied them) and were a deliberate, user-approved call. Seeing a page is not the same as acting on it: `boss` still cannot manage users (`_is_full_admin` gates that independently and was not touched), and reference-data writes stay `admin`/`director`/`export_manager`-only (`REFERENCE_DATA_WRITE`, also untouched — the frontend now hides those controls from him instead of rendering buttons that 403) — see the Resource CRUD Matrix footnote below. See [[boss]], [[permissions-system]], `docs/ADR.md` (AD-15).
