...
| [[shipment-creation]] | Create new shipments (shipment_code + date + country + customer). In the [[draft-shipments#Two-column Join flow (coexisting alternative)]] flow, creates the destination-only draft column and owns the **Join** action that merges a supply draft into it |
...
2. **Create shipment**: ShipmentList → Create button → fill shipment_code, date, country, customer
