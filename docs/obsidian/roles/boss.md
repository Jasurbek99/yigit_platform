...
The sidebar `boss` sees is **his own composition**, not the one every other role gets: `BOSS_MENU_GROUPS` is ordered by export process phase while `STAFF_MENU_GROUPS` keeps the original module grouping, and `pickMenuComposition()` selects between them by role. They are disjoint compositions over nearly the same route set — since 2026-08-20 the boss's is the staff set minus two, see below — full mechanism and the drift hazards in [[../processes/permissions-system#Sidebar Navigation (2026-08-05)]]. (This paragraph previously said the order was global and that `boss` got no special ordering; true when written, false since the owner asked for boss-only process ordering.) Per-role *configurable* ordering — a table plus a drag-drop admin UI — remains deferred.

**Withheld from the boss sidebar (2026-08-20, owner request):** `/export/drafts` (Draft Shipments) and `/export/assign` (Assignment Board) were removed from `BOSS_MENU_GROUPS`' `nav.group_prep`, which now holds `/export/weightmaster` alone. They remain in `STAFF_MENU_GROUPS` — they are working pages for `export_manager` / `loading_dept_head`. This is **sidebar-only**: the routes still resolve if typed, and the boss's backend page permissions and the `assign` action grant are untouched, because the planned process page mounts both screens as tabs. Both entries entered the boss menu through 421068f's "surface orphaned routes" sweep; a repeat of that sweep would silently re-add them, so the exclusion carries a comment at the group in `AppLayout.tsx` and an ordered-key assertion in `AppLayout.menuGroups.test.tsx`.
...
- `POST /shipments/{id}/assign/` — **now accepts `boss`** (widened at the call site, 2026-08-05). Assigning a draft is a real process step for him, so its only action had to work. The original rationale named the `/export/assign` sidebar entry; that entry was withdrawn on 2026-08-20 (see the withheld-pages note under [[#Page visibility]]) but the grant stands — the page is still reachable and the process page will drive the same action.
