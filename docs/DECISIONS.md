...
## ADR-0013: Статус `draft` для людей называется «Подготовка», код остаётся `draft`

**Дата:** 2026-09-24
**Статус:** accepted

### Контекст
Руководству не нравится слово «черновик» (Draft / Garalama). Оно было в названии
статуса в БД и примерно в 70 переводах на каждом из трёх языков.

### Решение
Меняем только то, что видят люди: названия статуса в БД (`core/0064`) — «Подготовка /
Preparation / Taýýarlyk», переводы tk/ru/en, тексты ошибок и уведомлений бэкенда.
«Черновик поставки / назначения» → «план поставки / назначения». Код `draft`, ключи i18n,
API и `phase='DRAFT'` не меняются.

### Альтернативы (что отвергли и почему)
- «Планирование» — отвергли: на доске уже есть отдельная пустая колонка «Планирование»,
  а шипменты `draft` стоят в колонке «Подготовка». Вышло бы «Планирование» внутри «Подготовки».
- Переименовать сам код `draft` — отвергли: на нём держатся переходы, правила задач,
  фильтр `?status_code=draft`, API и тесты, а пользователь код не видит.

### Последствия
- В коде и в vault слово *draft* остаётся — это имя кода, не текст для людей.
- «Черновик» у квот, плана местных продаж и ContractSale не тронут: там оно значит
  «не утверждено», это другое понятие.
- В tk колонка доски `phase.prep` выровнена с названием статуса («Taýýarlyk»).

### Дополнение 2026-09-26: имена частей шипмента
Владелец задал имена трёх частей. В tk заменено 31 значение («… meýilnamasy» → «… bölegi»),
ru и en уже совпадали и не менялись. Экран Gaplama (копия sera, «Tır Aç») не тронут.

| Часть | tk | ru | en |
|---|---|---|---|
| Поставка (Soltanmyrat: блоки, кг) | Üpjünçilik bölegi | план поставки | supply plan |
| Экспорт (Gadam: страна, клиент) | Eksport bölegi | план назначения | destination plan |
| Транспорт (в интерфейсе пока нет) | Transport bölegi | план транспорта | transport plan |
