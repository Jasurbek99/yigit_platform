...
**2026-05 amendment (revised)**: Only `customs_exit_at` is still written by `transition_to()` (on `'gumruk_chykysh'`). Every other lifecycle timestamp — `loading_started_at` (R19), `departed_at` (R21), `border_crossed_at` (R30), `customs_entry_at` (R32), `arrived_at` (R35), `sale_started_at` (R41), `sale_ended_at` (R42) — is operator-entered on the Sheet (`input_type='datetime'`). `STATUS_TIMESTAMP_MAP` shrinks to a single entry; `ShipmentPatchSerializer` accepts the seven operator-entered columns. AD-1's "single write path" invariant now applies to one column. Practical reason: warehouse, transport, and sales staff fill these times from physical events (door closed, gate stamp, sale concluded) that don't line up with the workflow's status transitions — they need to enter the actual time, not get whatever `timezone.now()` was at the moment they clicked "Next status".
