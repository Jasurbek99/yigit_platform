...
- Connects to Z_TIRWEB read-only via **pyodbc** (`SERVER=10.10.11.233,62079;DATABASE=Z_TIRWEB;UID=<user>;PWD=<secret>;TrustServerCertificate=yes`), from a small `services/tir_client.py` helper. Read-only — never writes to Z_TIRWEB.
