...
---
## UPDATE 2026-08-11 — SP3b DONE → TIR FEATURE COMPLETE
- **SP3b DONE** (reviewed, CRITICAL fixed + test-proven): the Sheet `truck_plate` cell is a virtual combined cell. Non-gapy → `SheetTruckSelectEditor.tsx` (two-select overlay + inline "+ Add", portaled to `document.body`) saving `truck_head_id`+`trailer_id`+derived `truck_plate` in one `patchMultiMutation` with Sheet undo; gapy → plain text `<Input>` (HARD RULE). Shared `composeTruckPlate()` util (`frontend/src/utils/truckPlate.ts`). Design spec `docs/superpowers/specs/2026-08-11-tir-fleet-sp3b-sheet-cell-design.md`; plan `docs/superpowers/plans/2026-08-11-tir-fleet-sp3b-sheet-cell.md`. Commits `a04b7dc..28abddb` (incl. `6d52850` = CRITICAL portal fix for the cell `contain: layout paint` clipping).
- **⚠ NEEDS a real-browser check:** the portal/positioning fix is a browser-layout concern jsdom can't verify — open the Sheet's `truck_plate` cell (non-gapy) and confirm the two-select panel renders un-clipped at the right spot. Logged in `BUILD_TEST_LOG.md`.
- **All four sub-projects (SP3a, SP3c, SP4, SP3b) are DONE and pushed** to `origin/feat/transport-fleet-map`. TIR fleet integration is feature-complete. The branch is **not yet merged / PR'd to main** — that's the next decision.
- Outstanding follow-up tickets (not built): stale `Shipment.truck_plate` on fleet-plate rename; backport the searchValue-clear fix to `CitySelect`/`CountrySelect`.
